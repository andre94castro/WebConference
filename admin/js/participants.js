const renderParticipants = async () => {
    ...
    let strHtml = `
        <thead> 
            <tr>
                <th class='w-100 text-center bg-warning' colspan='4'> Lista de Participantes</th>
            </tr>
            <tr class='bg-info'>
                <th class='w-2'>#</th>
...
for (let i = 0; i < btnDelete.length; i++) {
    btnDelete[i].addEventListener("click", () => {
        //invocação de janela modal para confirmar remoção
        //Janela modal
        swal({
            title: "Tem a certeza?",
            ...
            try{
                ...
                fetch(`${urlBase}/conferences/1/participants/${participantId}`, {
                    method: "DELETE"
                }) 
